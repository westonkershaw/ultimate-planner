...
* [x] Confirm the data layer in use (Supabase or other) and note where auth stands (target: Google + Apple + email/password). If auth is incomplete, finish it FIRST as "Phase 0.5" on `feature/auth-completion` before Phase 1 — everything downstream assumes real accounts — CONFIRMED: Supabase (email/password LIVE with RLS'd learning tables; Google/Apple code wired but providers NOT enabled → Phase 0.5 required, needs Weston's Google Cloud + Apple Developer credentials; note: core user data is still localStorage-per-device, only the learning profile syncs)
* [ ] Establish the time policy in one shared util: all "day" boundaries (streaks, Today chips, nightly ritual, newlywed year) computed in the device's local timezone — never UTC. Every later phase imports this instead of rolling its own — `src/utils/timePolicy.ts` + tests WRITTEN 2026-07-18; tsc/test verification pending (tool outage); known offenders to migrate later: usePlannerStore/WeekWizard use UTC `toISOString().slice(0,10)`
* [ ] Verify `.claude/agents/` contains implementer / code-reviewer / test-writer pinned to Sonnet; create them if missing — MISSING; creation blocked by tool outage 2026-07-18 (content drafted, retry next run; note: agent files load at session start)
