    ...
    >
      ...
      <button
        ...
        className={[
          ...
          selected ? 'bg-accent hover:bg-accent-hover text-white' : 'bg-surface-2 text-fg-faint cursor-not-allowed',
        ].join(' ')}
      >
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {m && <span className="text-[10px] text-fg-muted">{m.label}</span>}
        </div>
        {entry.note && <p className="text-xs text-fg-muted mt-1 leading-relaxed whitespace-pre-wrap">{entry.note}</p>}
      </div>
      <button
...
export default function MoodView() {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-2xl mx-auto space-y-6 pb-24">
      ...
      >
        <Smile size={18} className="text-accent-text" />
        <h1 className="font-syne text-lg font-bold text-fg">Mood</h1>
      </motion.div>
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <StatPill Icon={({ size }) => <Smile size={size} className="text-accent-text" />} label="Entries" value={String(entries.length)} />
      </div>
