...
export default function BackupSettings() {
  ...
  return (
    <div className="space-y-4">
      <label className="text-xs font-medium text-slate-500 uppercase tracking-wider">Backup & Restore</label>
      ...
      <div className="flex gap-2">
        <motion.button
          ...
          className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border border-slate-800/60 bg-slate-900/40 text-sm text-slate-300 hover:border-indigo-500/30 transition-colors"
        >
      ...
      {/* Google Drive */}
      <div className="p-3 rounded-xl border border-slate-800/40 bg-slate-900/30 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Cloud size={15} className={googleConnected ? 'text-emerald-400' : 'text-slate-600'} />
            <span className="text-sm text-slate-300">Google Drive</span>
          </div>
          <span className={`text-[10px] px-2 py-0.5 rounded-full ${googleConnected ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-800 text-slate-500'}`}>
            {googleConnected ? 'Connected' : 'Not connected'}
          </span>
        </div>
        <button className="w-full py-2 rounded-lg border border-slate-800/60 text-xs text-slate-400 hover:text-slate-200 hover:border-indigo-500/30 transition-colors">
      ...
      {/* Auto backup */}
      <div className="flex items-center justify-between">
        <span className="text-sm text-slate-400">Auto-backup</span>
        <button
          onClick={() => setAutoBackup(!autoBackupEnabled)}
          className={`w-10 h-5 rounded-full transition-colors relative ${autoBackupEnabled ? 'bg-indigo-500' : 'bg-slate-700'}`}
        >
          <motion.div
